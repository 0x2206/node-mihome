...
export default class extends Device {
  ...
  setPower(v: boolean) {
    return this.miioCall("set_power", utils.withLightEffect(v ? "on" : "off"));
  }

  setBrightness(v) {
    return this.miioCall("set_bright", utils.withLightEffect(v));
  }

  setColorTemperature(v) {
    return this.miioCall("set_ct_abx", utils.withLightEffect(v));
  }

  setSleepMode(v: boolean) {
