...
class MiIOProtocol extends EventEmitter {
  ...
  createSocket() {
    ...
    this._socket.on("listening", () => {
      ...
      const address = this._socket.address() as AddressInfo; //@TODO avoid "as"
  ...
  _decryptMessage(address: string, msg: Buffer) {
    ...
    const digest = crypto.createHash("md5").update(msg.slice(0, 16)).update(device._token).update(encrypted).digest();
  ...
  _encryptMessage(address: string, data) {
    const device = this.getDevice(address);
  ...
  _onData(address: string, msg: Buffer) {
  ...
  _socketSend(msg: Buffer, address: string, port = PORT) {
  ...
  async send(address: string, method, params = [], options = {}) {
...
export default new MiIOProtocol();
