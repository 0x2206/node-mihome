...
class AqaraProtocol extends EventEmitter {
  private _gateways = new Map();
  private _gatewaysByAddress = new Map();
  private _subDevices = new Map();
  ...
  generateKey(password: string, token: Buffer): string | null {
    let key = null;
    if (password && token) {
  ...
  _send(data, port: number, address: string) {
...
export default new AqaraProtocol();
